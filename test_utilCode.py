...
def test_tendanceRatioDetteCAF(dictDette, dictCAF, ch):
    ...
    dictRatio, strTendance = utilCode.tendanceRatioDetteCAF(config, dictCAF, dictDette,
                                                            True, True)
...
def test_presentRatioDettesCAF(ratio, ch):
    ...
    config = configparser.RawConfigParser()
    config.read('FinancesLocales.properties')
    assert ch in utilCode.presentRatioDettesCAF(config, ratio, True, True)
